...
public static class ApplicationRestart
{
    public static bool TryRestartApplication(string reason = "unspecified", Window? owner = null)
    {
        try
        {
            ...
            if (!WorkstationNetwork.TryScheduleRestart(exePath, AppContext.BaseDirectory, reason))
                return false;
    ...
    {
        if (ApplicationRestart.TryRestartApplication("workstation-role-change", owner))
            return true;

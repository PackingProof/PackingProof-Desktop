    ...
    public partial class MainViewModel
    {
            ...
            int w, int h, int fps, string encoder, bool withAudio, string? audioPipeName = null)
        {
            ...
            try
            {
                ...
                // 时间轴自检：fileSeconds 应当与 wallSeconds 接近（都含预录那一段），
                // 差得多就说明录像被快放或慢放了。
                if (writtenFrames > 0)
                {
                    double fileSeconds = RecordingTimelinePolicy.CalculateFileSeconds(writtenFrames, fps);
                    double wallSeconds = RecordingTimelinePolicy.CalculateWallSeconds(
                        preRecordFrames,
                        fps,
            ...
            string? audioPipeName = null)
        {
            ...
            if (withAudio)
                ...
                args += " -map 0:v:0 -map 1:a:0 -c:a aac -profile:a aac_low -b:a 128k -af aresample=async=1:first_pts=0,apad -shortest -avoid_negative_ts make_zero";

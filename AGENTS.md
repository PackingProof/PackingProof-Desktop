...
- 发布笔记必须使用仓库根目录的 `RELEASE_NOTES_TEMPLATE.md` 模板：更新内容按“功能与体验 / 问题修复 / 兼容与工程”三类填写，并包含下载与更新说明、未验证事项；禁止自创格式。Release 标题固定为“`v<X.Y.Z> <一句话内容>`”（版本号开头，不加产品名或“发布”等前缀）。`update_vX.Y.Z.json` 的 `title` 与 `notes` 必须与最终发布说明同步。

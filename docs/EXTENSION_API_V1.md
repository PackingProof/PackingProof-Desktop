...
油猴脚本还可以通过“扩展市场”安装，或从本地导入 `.user.js`、`.ppext`，再通过“安装订单联动”选择安装。导入和安装只负责维护脚本文件、地址及更新链接，不会绕过扩展授权。市场扩展建议把市场扩展 ID 同时用作 `providerId`，每台安装实例仍应生成独立的 `extensionInstanceId`
...
旧脚本与市场脚本因 `@name` 或 `@namespace` 不同而同时运行时，旧订单接口会在三秒兼容窗口内合并同一来源、同一目标和语义完全相同的非测试请求。重复请求会等待并复用首个请求的真实处理结果，广播请求在向录像主机转发前完成合并；首个请求失败时，所有等待方收到同一失败且后续可以立即重试。测试订单、内容发生变化、来源或目标不同以及窗口外的再次推送仍会正常处理。同一来源同时出现有版本和无版本的官方脚本心跳时，安装入口会提示用户删除旧脚本。
...
扩展 API 的启用状态、能力查询、服务启动方式和签名鉴权失败原因会默认写入自动轮转的 `runtime.log`，使用“关于 → 导出诊断日志”时会一并收集，不需要用户提前打开“详细调试日志”。设置中的详细调试日志可能包含订单内容和文件路径，只应在需要复现业务数据或媒体处理问题时临时开启。

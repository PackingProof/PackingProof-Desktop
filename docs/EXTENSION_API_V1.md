...
旧脚本与市场脚本因 `@name` 或 `@namespace` 不同而同时运行时，旧订单接口会在三秒兼容窗口内忽略语义完全相同的非测试订单。测试订单、内容发生变化的订单和窗口外的再次推送仍会正常处理；处理失败会释放本次去重占位，允许脚本安全重试。同一来源同时出现有版本和无版本的官方脚本心跳时，安装入口会提示用户删除旧脚本。
